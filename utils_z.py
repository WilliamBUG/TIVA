...
def concat_envs_z(con_envs, device, expansion=False):
    ...
    if expansion is True:
        con_z = con_z.to(torch.double)
        con_z_tile = torch.tile(con_z, (1, 4))
        con_z_tile = torch.normal(mean=con_z_tile, std=0.5)
        con_z = torch.cat([con_z, con_z_tile], -1)
    con_c = torch.cat([env["color"] for env in con_envs])
    con_inv = torch.cat([env["noise"] for env in con_envs])
    con_z = con_z[:, 1].view(-1, 1)
    con_z = con_z.repeat(1, 5)
    return con_x.to(device), con_y.to(device), con_z.to(device), con_g.to(device), con_c.to(device), con_inv.to(device)
...
class LOGITZ(object):
    ...
    def preprocess_data(self):
        self.train_x, self.train_y, self.train_z, self.train_g, self.train_c, self.train_invnoise= concat_envs_z(self.envs[:self.flags.envs_num_train], self.device, expansion=True)
        self.test_x, self.test_y, self.test_z, self.test_g, self.test_c, self.test_invnoise = concat_envs_z(self.envs[self.flags.envs_num_train:], self.device, expansion=True)
...
class CELEBAZ_FEATURE(object):
    ...
    def preprocess_data(self):
        ...
        train_file = F"train_{train_num}_{self.flags.cons_train}_{test_num}_{self.flags.cons_test}.csv"
        #, self.train_invnoise
        self.train_x, self.train_y, self.train_z, self.train_g, self.train_c= process_file(train_file)
        test_file = F"test_{train_num}_{self.flags.cons_train}_{test_num}_{self.flags.cons_test}.csv"
        print(train_file)
...
class HousePrice(object):
    ...
    def preprocess_data(self):
        mypath = "house_data_precessed.csv"
...
class LANDCOVER(object):
    def __init__(self, flags):
        ...
        config = dict()
        config['train_transforms'] = [{'classname': 'innout.datasets.transforms.LambdaTransform',
        ...
        config['test_transforms'] = [{'classname': 'innout.datasets.transforms.LambdaTransform',
                                      'args': {'function_path': 'innout.datasets.transforms.to_tensor'}},
                                     {'classname': 'innout.datasets.transforms.LambdaTransform',
                                      'args': {'function_path': 'innout.datasets.transforms.tensor_to_float'}}]
        config['dataset'] = {'classname': 'innout.datasets.landcover.Landcover',
                             'args': {'root': '/u/nlp/data/landcover/timeseries_by_box_v2',
                                      'cache_path': 'datasets/landcover.pkl', 'include_NDVI': True,
                                      'include_ERA5': True, 'standardize': True, 'shuffle_domains': True, 'seed': 1,
                                      'use_cache': True, 'use_unlabeled_id': False, 'use_unlabeled_ood': False,
                                      'unlabeled_prop': 0.9, 'pretrain': False, 'multitask': False},
                             'train_args': {'split': 'nonafrica-train'},
                             'eval_train_args': {'split': 'nonafrica-train'}, 'val_args': {'split': 'nonafrica-val'},
                             'test_args': {'split': 'nonafrica-test'}, 'test2_args': {'split': 'africa'}}
