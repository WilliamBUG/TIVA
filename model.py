...
class MLPIB2Layer(nn.Module):
    def __init__(self, flags, feature_dim, hidden_dim, latent_dim):
        super(MLPIB2Layer, self).__init__()
        self.flags = flags
        self.lin1 = nn.Linear(feature_dim, hidden_dim)
        self.enc_mean = nn.Linear(hidden_dim, latent_dim)
        self.enc_std = nn.Linear(hidden_dim, latent_dim)
        self.lin2 = nn.Linear(latent_dim, hidden_dim)
        self.lin3 = nn.Linear(hidden_dim, 1)

        for lin in [self.lin1, self.lin2, self.lin3, self.enc_mean, self.enc_std]:
          nn.init.xavier_uniform_(lin.weight)
          nn.init.zeros_(lin.bias)
    ...
    def forward(self, input):
        x1 = F.relu(self.lin1(input))
        mean = self.enc_mean(x1)
        std = F.softplus(self.enc_std(x1))

        # latent_dist = Normal(mean, std)
        # latent = latent_dist.sample()
        # latent = mean
        latent = self.reparametrize_n(mean, std)
        out = self.lin3(F.relu(self.lin2(latent)))
        return mean, std, out
...
class DensityEstimator(nn.Module):
    def __init__(self, flags, latent_dim, latent_dim_style, latent_dim_class):
        super(DensityEstimator, self).__init__()
        self.enc_sty_1 = nn.Linear(latent_dim, flags.hidden_dim_infer)
        self.enc_sty_2 = nn.Linear(flags.hidden_dim_infer, latent_dim)

        self.enc_class_1 = nn.Linear(latent_dim, flags.hidden_dim_infer)
        self.enc_class_2 = nn.Linear(flags.hidden_dim_infer, latent_dim)

        self.dec_fc_lin1 = nn.Linear(latent_dim_style + latent_dim_class, flags.hidden_dim_infer)
        self.dec_fc_lin2 = nn.Linear(flags.hidden_dim_infer, 1)

        for lin in [self.enc_sty_1, self.enc_sty_2, self.enc_class_1, self.enc_class_2, self.dec_fc_lin1, self.dec_fc_lin2]:
            nn.init.xavier_uniform_(lin.weight)
            nn.init.zeros_(lin.bias)

        self.sty_fc = nn.Sequential(self.enc_sty_1, nn.ReLU(True), self.enc_sty_2)
        self.class_fc = nn.Sequential(self.enc_class_1, nn.ReLU(True), self.enc_class_2)

        self.fc = nn.Sequential(self.dec_fc_lin1, nn.ReLU(True), self.dec_fc_lin2)

    def _call_method(self, style_emb, class_emb):
        #style_emb = self.sty_fc(style_emb)
        style_emb = style_emb
        #class_emb = self.class_fc(class_emb)
        class_emb = class_emb
        # return self.fc(torch.cat([style_emb, class_emb], dim=1))
        return torch.cat([style_emb, class_emb], dim=1)
    ...
    def forward(self, style_emb, class_emb, mode):
        ...
        # 2. q(s)q(t)
            ...
            # Permutation
            style_emb_permed = style_emb[torch.randperm(style_emb.size(0))]
            class_emb_permed = class_emb[torch.randperm(class_emb.size(0))]
...
def mmd_loss(source_features, target_features, device):
    ...
    loss_value = maximum_mean_discrepancy(source_features, target_features, kernel=gaussian_kernel).to(device)
